...
pub fn delimiterStringToFloat64(
    input: std.ArrayList([]u8),
    delimiter: []const u8,
    allocator: anytype,
) !std.ArrayList([]f64) {
    ...
    for (input.items) |line| {
        ...
        // Turn our dynamic list of f64 into an owned slice, then store it in the final result.
        const owned_slice: []f64 = try numbers.toOwnedSlice();
        try result.append(owned_slice);
...
pub fn delimiterStringToFloat32(
    input: std.ArrayList([]u8),
    delimiter: []const u8,
    allocator: anytype,
) !std.ArrayList([]f32) {
    ...
    for (input.items) |line| {
        ...
        // Turn our dynamic list of f32 into an owned slice, then store it in the final result.
        const owned_slice: []f32 = try numbers.toOwnedSlice();
        try result.append(owned_slice);
